...
class TaskTestSuite(pythia.TestSuite):
    ...
    def studentCode(self, data):
        find = [f for (n, f) in inspect.getmembers(q1, inspect.isfunction) if n == 'sayhello']
        if len(find) != 1:
            raise pythia.UndeclaredException('sayhello')
        if not callable(find[0]):
            raise pythia.BadTypeException('sayhello', type(find[0]), 'function')
        spec = inspect.getargspec(find[0])
        if len(spec.args) != 1:
            raise pythia.WrongParameterNumberException('sayhello', len(spec.args), 1)
        return q1.sayhello(data)
