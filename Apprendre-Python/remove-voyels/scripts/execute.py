...
class TaskTestSuite(pythia.TestSuite):
    ...
    def studentCode(self, data):
        find = [f for (n, f) in inspect.getmembers(q1, inspect.isfunction) if n == 'removevoyels']
        if len(find) != 1:
            raise pythia.UndeclaredException('removevoyels')
        if not callable(find[0]):
            raise pythia.BadTypeException('removevoyels', type(find[0]), 'function')
        spec = inspect.getargspec(find[0])
        if len(spec.args) != 1:
            raise pythia.WrongParameterNumberException('removevoyels', len(spec.args), 1)
        return q1.removevoyels(data)
